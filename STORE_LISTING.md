...
1. 作品ページでのOP/EDボタン確認
   https://animestore.docomo.ne.jp/animestore/ci_pc?workId=27008 を開く
   → エピソード行に「OP/ED」ボタンが表示されること
   → クリックで新規ウィンドウ/タブが開くことを確認

2. ポップアップUI
   ツールバーの拡張機能アイコンをクリック
   → 空状態「プレイリストがありません。」＋「管理画面で作成」ボタン表示
   → 「管理」ボタンでオプションページが開くこと
   → フッターにバージョン表示があること

3. オプションページ（プレイリスト管理）
   test/sample_playlist.json をJSONインポート
   → プレイリストの展開/折りたたみ、曲数・合計時間表示、名前編集
   → アイテムの▶個別再生、編集（範囲名・時間）、コピー、削除
   → ドラッグ＆ドロップでのアイテム並び替え
   → JSONエクスポート（ファイルダウンロード）
   → 「新規ウィンドウ/新規タブ」設定切替

本拡張機能の全コードはパッケージ内に同梱されており、リモートコードは使用していません。
サンプルJSON: https://gist.github.com/sas-news/99d6ee6fec905d3de2c071654167f068
```
