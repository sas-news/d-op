...
| 権限 | 使用理由 |
|---|---|
| `scripting` | 動画プレイヤーを制御するためのスクリプト注入に必要です |
| `tabs` | プレイリスト再生時に新規ウィンドウ/タブを開くために必要です |
| `storage` | プレイリストや設定をブラウザのローカルストレージに保存するために必要です |
| ホスト権限 (`animestore.docomo.ne.jp`, `anime.dmkt-sp.jp`) | dアニメストアの作品ページとプレイヤーページにUIを追加するために必要です |
